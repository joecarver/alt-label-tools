      ...
      `)
        ...
        const statusUpdates = await Promise.all(
            tasks.map(async task => {
                const release = Array.isArray(task.releases) ? task.releases[0] : task.releases
                const status = await getTaskStatus(task.id, task.name, release.folder_id, task.completed_at, task.is_detectable, task.end_date)
        ...
        const result = {
            ...
            recordsCreated: statusUpdates.filter(s => !existingIds.has(s.taskId)).length,
            recordsUpdated: statusUpdates.filter(s => existingIds.has(s.taskId)).length
        }

        await trackSyncExecution('sync-task-statuses', 'success', result, null, startTime)

        console.log(`Successfully updated ${result.recordsProcessed} task statuses`)
        return successResponse(result)
    } catch (error) {
        const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred'
        await trackSyncExecution('sync-task-statuses', 'failure', {
            recordsProcessed: 0,
            recordsCreated: 0,
            recordsUpdated: 0
