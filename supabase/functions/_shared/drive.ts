...
async function generateServiceAccountToken() {
  ...
  const header = {
    alg: "RS256",
    typ: "JWT",
    kid: Deno.env.get("GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY_ID"),
  };
...
export async function listFilesInFolder(
  ...
  try {
    ...
    const response = await fetch(
      `https://www.googleapis.com/drive/v3/files?q='${folderId}'+in+parents+and+trashed+=+false&fields=files(id,name,webViewLink,mimeType,createdTime,modifiedTime)`,
      {
        headers: {
    ...
    const files: DriveFile[] = data.files.filter((file: DriveFile) =>
      permittedMimeTypes?.includes(file.mimeType ?? "")
    );
